...
function fetchEntrypointAndDocs(entrypointUrl, options = {}) {
  return fetchJson(entrypointUrl, options).then(d => {
    return {
      entrypointUrl,
    ...
    fetchJson(data.docsUrl, options).then(d => {
      data.docs = d.body;

      return data;
    }).then(data =>
      promises.expand(data.docs, { base: data.docsUrl, documentLoader: (input) => fetchJson(input, options) }).then(docs => {
        data.docs = docs;

        return data;
      })
    ).then(data =>
        promises.expand(data.entrypoint, { base: data.entrypointUrl, documentLoader: (input) => fetchJson(input, options) }).then(entrypoint => {
...
/**
 * Parses a OpenApi documentation and converts it to an intermediate representation.
 *
 * @param {string} entrypointUrl
 * @param {object} options
 * @return {Promise.<Api>}
 */
export default function parseOpenApiDocumentation(entrypointUrl, options = {}) {
